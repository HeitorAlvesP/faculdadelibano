...
import EducacaoList from "./CourseList/EducacaoList";
import SaudeList from "./CourseList/SaudeList";
import EngenhariaList from "./CourseList/EngenhariaList";
import DireitoList from "./CourseList/DireitoList";
import PsicologiaList from "./CourseList/PsicologiaList";
import EmpresarialList from "./CourseList/EmpresarialList";
import MbaExecutivoList from "./CourseList/MbaExecutivoList";
import MeioAmbienteList from "./CourseList/MeioAmbienteList";
import ServicoSocialList from "./CourseList/ServicoSocialList";
...
export default function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  const renderActiveList = () => {
    switch (activeCategory) {
      case 'Educação':
        return <EducacaoList onBack={() => setActiveCategory(null)} />;
  
      case 'Saúde':
        return <SaudeList onBack={() => setActiveCategory(null)} />;
      
      case 'Engenharia':
        return <EngenhariaList onBack={() => setActiveCategory(null)} />;
      
      case 'Direito':
        return <DireitoList onBack={() => setActiveCategory(null)} />;

      case 'Psicologia': 
        return <PsicologiaList onBack={() => setActiveCategory(null)} />;

      case 'Empresarial':
        return <EmpresarialList onBack={() => setActiveCategory(null)} />;
      
      case 'MBA Executivo':
        return <MbaExecutivoList onBack={() => setActiveCategory(null)} />;
      
      case 'Meio Ambiente':
        return <MeioAmbienteList onBack={() => setActiveCategory(null)} />;

      case 'Serviço Social':
        return <ServicoSocialList onBack={() => setActiveCategory(null)} />;
      
      default:
        return (
          <div className="flex flex-col items-center justify-center h-full w-full bg-white p-10 text-center rounded-[24px]">
            <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" className="text-[#ea005f] w-16 h-16 mb-4" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            <h3 className="text-xl font-bold text-[#0B182F] mb-3">Lista em desenvolvimento</h3>
            <p className="text-gray-500 mb-8 max-w-sm">
              O arquivo da lista de cursos para a área de <strong className="text-[#0B182F]">{activeCategory}</strong> será adicionado em breve.
            </p>
            <button 
              onClick={() => setActiveCategory(null)} 
              className="px-8 py-3 bg-[#ea005f] hover:bg-[#c90050] transition-colors text-white rounded-full font-bold shadow-md"
            >
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-start justify-center pt-[5vh] bg-[#0B182F]/50 backdrop-blur-md px-4 pb-4 animate-in fade-in duration-300">
      ...
      <div className="bg-white w-[95%] max-w-[1300px] min-h-[60vh] max-h-[90vh] rounded-[24px] shadow-2xl flex flex-col overflow-hidden transform transition-all relative">
        ...
        {activeCategory ? (
          ...
          <>
            ...
            <div className="p-6 md:p-10 bg-gray-50/50 overflow-y-auto md:overflow-hidden flex-1 flex flex-col justify-between no-scrollbar">
